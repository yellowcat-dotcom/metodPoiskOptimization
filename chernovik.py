...
#  версия где работает лучше но не то что надо
    ...
    def select(self):
        ...
        new_population = sorted_pop[:cutoff] + children

        sorted_new_pop = sorted(new_population, key=lambda item: item[2], reverse=not self.min_func)
        print(len(sorted_new_pop))

        self.population = sorted_new_pop[:cutoff]
...
class GeneticAlgorithmL3:
    ...
    def select(self):
        ...
        sorted_new_pop = sorted(new_population)
        print(len(sorted_new_pop))

        self.population = sorted_new_pop
