...
class rastriginbee(pybee.floatbee):
    ...
    @staticmethod
    def getstartrange():
        return [-5.12] * rastriginbee.count

    @staticmethod
    def getrangekoeff():
        return [0.98] * rastriginbee.count

    def __init__(self):
        pybee.floatbee.__init__(self)

        self.minval = [-5.12] * rastriginbee.count
        self.maxval = [5.12] * rastriginbee.count
    ...
    def calcfitness(self):
        ...
        self.fitness = 0.0
        for n in range(rastriginbee.count):
            xi = self.position[n]
            self.fitness += -(xi ** 2 - 10 * math.cos(2 * math.pi * xi) + 10)
